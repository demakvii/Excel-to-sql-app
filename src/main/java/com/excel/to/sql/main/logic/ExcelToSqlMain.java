package com.excel.to.sql.main.logic;
...
import com.excel.to.sql.main.pojo.Header;

public class ExcelToSqlMain {

	@SuppressWarnings("resource")
	public static byte[] convertExcelToSql(Boolean isMapped, byte[] inputFile,
			byte[] outputFile) throws IOException {
		InputStream inFile = new ByteArrayInputStream(inputFile);
		InputStream ouFile = new ByteArrayInputStream(outputFile);
		...
		StringBuffer outQuey = new StringBuffer();
		if (outputResult != null && !outputResult.isEmpty()) {

			for (String sqlQuery : outputResult) {
				for (String sqlPart : sqlQuery.split(";"))
					if (sqlPart.length() > 0)
						outQuey.append(sqlPart + ";" + "\n\n");
			}
		}
		...
		inFile.close();
		return outQuey.toString().getBytes();
	}
